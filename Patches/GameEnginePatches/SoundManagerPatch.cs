...
namespace LethalBots.Patches.GameEnginePatches
{
    ...
    {
        /// <summary>
        /// Patch for only set player pitch for not bot
        /// </summary>
        /// <remarks>
        /// Actually, allow this to happen now that we have proper support for it!
        /// </remarks>
        ...
        {
            LethalBotAI? lethalBotAI = LethalBotManager.Instance.GetLethalBotAI(playerObjNum);
            if (lethalBotAI != null)
            {
                return false;
            }
        ...
        public static IEnumerable<CodeInstruction> SetPlayerVoiceFilters_Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            {
                Plugin.LogError($"LethalBot.Patches.GameEnginePatches.SoundManagerPatch.SetPlayerVoiceFilters_Transpiler could not use irl number of player in list.");
            }
