...
namespace LethalBots.Patches.ModPatches.BetterEmotes
{
    public class BetterEmotesPatch
    {
        ...
        public static IEnumerable<CodeInstruction> UpdatePostfix_Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            // ----------------------------------------------------------------------
            Label labelToJumpToLethalBotOwnedLocal = generator.DefineLabel();
            // ldarg.0 NULL after set_runtimeAnimatorController playerBodyAnimator others
            codes[startIndex + 25].labels.Add(labelToJumpToLethalBotOwnedLocal);

            Label labelToJumpToLethalBotNotOwnedLocal = generator.DefineLabel();
            // ldarg.0 NULL before set_runtimeAnimatorController playerBodyAnimator others
            codes[startIndex + 20].labels.Add(labelToJumpToLethalBotNotOwnedLocal);

            // Use runtimeAnimatorController local for bots owned by player local
            List<CodeInstruction> codesToAdd = new List<CodeInstruction>
            {
                new CodeInstruction(OpCodes.Ldarg_0),
                new CodeInstruction(OpCodes.Call, PatchesUtil.IsPlayerLethalBotOwnerLocalMethod),
                new CodeInstruction(OpCodes.Brtrue, labelToJumpToLethalBotOwnedLocal),
                new CodeInstruction(OpCodes.Ldarg_0),
                new CodeInstruction(OpCodes.Call, PatchesUtil.IsPlayerLethalBotMethod),
                new CodeInstruction(OpCodes.Brtrue, labelToJumpToLethalBotNotOwnedLocal),
            };
        ...
        public static IEnumerable<CodeInstruction> PerformEmotePrefix_Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            if (startIndex > -1)
            {
                ...
                List<CodeInstruction> codesToAdd = new List<CodeInstruction>
                {
                    ...
                    new CodeInstruction(OpCodes.Call, PatchesUtil.IsPlayerLethalBotControlledAndOwnerMethod),
            ...
            {
                Plugin.LogError($"LethalBot.Patches.ModPatches.BetterEmotesPatch.PerformEmotePrefix_Transpiler could not use irl number of player for iteration.");
            }
            ...
            {
                Plugin.LogError($"LethalBot.Patches.ModPatches.BetterEmotesPatch.PerformEmotePrefix_Transpiler could not bypass context performed for bots.");
            }
            ...
            {
                Plugin.LogError($"LethalBot.Patches.ModPatches.BetterEmotesPatch.PerformEmotePrefix_Transpiler could not bypass timeSinceStartingEmote for bots.");
            }
